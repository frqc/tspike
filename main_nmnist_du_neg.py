...
@click.option('-c', '--background', default=0.0001)
...
def main(
    gpu, batch, epochs, supervised, background,
    ...
    **kwargs
    ...
    model = FullDualColumn(
        ...
        step=step, leak=leak, bias=bias, background=background,
        dense=dense, fodep=forced_dep, w_init=w_init
    ).to(device)

    model.load_state_dict(torch.load('20_4-e-4'))
    print("Finished loading ", '20_4-e-4')
    ...
    for epoch in range(epochs):
        ...
        # epoch_model_path = model_path + "-e-" + str(epoch)
        # torch.save(model.state_dict(), epoch_model_path)
        # print("saving", epoch_model_path)
